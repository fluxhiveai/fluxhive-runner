 ...
 *
 * Connects to the OpenClaw gateway via WebSocket and uses a request/response
 * protocol (JSON frames with `type: "req"/"res"/"event"`). Handles:
 *   - Device identity authentication (ED25519 keypair + challenge/response)
 *   - Token priority: stored device token → env token → auto-detected gateway token
 *   - Automatic token fallback on "device token mismatch"
 *   - Request timeouts and pending-request tracking
 *   - Agent execution with payload extraction and abort support
 *
 * The gateway serves as a bridge to run AI agents (Claude, etc.) without
 * needing direct API keys — the gateway handles model routing and billing.
 */
...
/** A pending request awaiting a gateway response frame. */
type Pending = {
...
/** Server-initiated event (e.g. connect.challenge, agent progress). */
...
export class OpenClawClient extends EventEmitter {
   ...
   * Routes event frames (like connect.challenge) to event handlers,
   * and response frames to their pending promise resolvers.
   */
  ...
  /** Resolves the shared (non-device-specific) auth token from options or gateway config. */
  private resolveSharedToken(): string {
    return this.opts.token || loadGatewayToken() || "";
  }

  /**
   * Sends the "connect" handshake to the gateway with device identity auth.
   * If a connect.challenge event provided a nonce, it's included for replay protection.
   * On device token mismatch, clears the stored token and retries with the shared token.
   */
  ...
  /**
   * Establishes the WebSocket connection and completes the handshake.
   * If already connected, returns immediately. If a connect is in progress,
   * awaits the existing promise to avoid duplicate connections.
   */
  ...
  /**
   * Sends a JSON-RPC-style request to the gateway and awaits the response.
   * Auto-connects if needed. Supports timeouts and "expectFinal" mode
   * (where an intermediate "accepted" response is ignored, waiting for the final result).
   */
  ...
  /**
   * Runs an agent execution via the gateway.
   * Sends the prompt to the specified agent, waits for completion,
   * and extracts text payloads + usage metrics from the response.
